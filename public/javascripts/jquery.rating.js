...
/*# AVOID COLLISIONS #*/
	...
	$.fn.rating = function(instanceSettings){
		...
		this.each(function(i){
			...
			var original_name = (this.name || 'unnamed-rating');
			var n = original_name.replace(/\]$/,'').replace(/\[|\]/g, "_");
			...
			if(i == 0){
				...
				$.rating.groups[n].valueElem = $('<input type="hidden" name="' + original_name + '" value=""' + (settings.readOnly ? ' disabled="disabled"' : '') + '/>');
